...
namespace WcfServiceLibrary1
{
    ...
    public class Ram : SRam
    {
        ...
        public void Save()
        {
            ...
            string SQL = "INSERT INTO public.ram(manufacturer, model, memory, clock_freq, memory_type, is_del) VALUES(@manufacturer, @model, @memory, @clock_freq, @memory_type, false);";
        ...
        public string get()
        {
            ...
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;User Id=Admin;Password=Admin;Database=computers;");
            NpgsqlCommand command = new NpgsqlCommand("SELECT manufacturer.name, ram.model, ram.memory, ram.clock_freq, ram.memory_type FROM public.ram, public.manufacturer WHERE (ram.is_del=false) and ram.manufacturer = manufacturer.id LIMIT 1000", conn);
            NpgsqlDataReader reader = null;
            try
            {
                conn.Open();
        ...
        public void delete(string model)
        {
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;User Id=Admin;Password=Admin;Database=computers;");
            NpgsqlCommand command = new NpgsqlCommand();
            string SQL = "UPDATE public.ram SET is_del = true WHERE model=@model;";
            command.CommandText = SQL;
            command.Parameters.AddWithValue("@model", model);
            command.Connection = conn;
            NpgsqlDataReader reader = null;
